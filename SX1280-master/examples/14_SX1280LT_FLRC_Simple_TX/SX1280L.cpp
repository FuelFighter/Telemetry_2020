...
bool SX1280Class::sendPacketFLRC(uint8_t *TXbuffer, uint8_t size, int32_t txtimeoutmS, int8_t TXpower, uint8_t _DIO)
{
  ...
  //THIS IS SOME maybe FUCKING BULLSHIT!!!!
  while (!digitalRead(_DIO));                                     //Wait for DIO to go high
...
#ifdef SX1280DEBUG
  Serial.println(F("ReadRegisters - 1426: "));
...
{
#ifdef SX1280DEBUG
  Serial.println(F("writeRegisters() - 1569 EYO COWBOY"));
#endif
  uint8_t addr_l, addr_h;
  uint8_t i;
  ...
  for (i = 0; i < size; i++)
  {
        ...
        Serial.print(" This bitch on 1594");
    #endif
